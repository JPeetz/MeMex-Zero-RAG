...
## 2026-06-02 — AgentForge Open-Source Framework Launch

### Decision: Architecture Spec + Scaffolding Complete

**Context:** Board directed creation of open-source AgentForge framework — Go-based, capability-secured, multi-OS (TUI + web GUI mandatory, native OS app later, MCP native).

**What was done:**
- Full architecture specification (ARCHITECTURE.md, 15K words) — module map, data flow, component interfaces, security model, MeMex memory schema
- Project scaffolding: Go module (go 1.24), Makefile, Dockerfile, docker-compose, CI/CD (GitHub Actions), .gitignore
- Core source code (compiles, 10MB static binary):
  - `cmd/agentforge/main.go` — CLI entry point (run/daemon/spawn/version)
  - `internal/bus/bus.go` — CSP message bus (pub/sub, request/reply, broadcast)
  - `internal/config/config.go` — YAML/ENV/CLI loader
  - `internal/engine/agent.go` — Goroutine agent pool, department pools, DAG pipeline
  - `internal/memory/store.go` — MeMex Zero RAG (git + SQLite FTS5 + file watcher)
  - `internal/security/capability.go` — HMAC-SHA256 capability tokens, enforcer
- Documentation: README (6.5K words), CHANGELOG, CONTRIBUTING, LICENSE (BUSL-1.1), docs index
- Path: `~/workspace/AgentForge/`

**Next:** Week 2 — LLM adapters, tool registry, MCP server, subagent trees, Docker integration

- [2026-06-06] [DECISION — CORRECTION]: The 2026-06-02 "AgentForge Open-Source Framework Launch" decision was partially incorrect. The Go framework DID exist (68 files, 29.7K LOC, compiles to 14MB binary) but was located at `~/.openclaw/workspace/agentforge/` — hidden inside OpenClaw's private directory, not the visible workspace. This was discovered during a CEO codebase audit triggered by Joerg's CodeFlow repo assessment. **Resolved:** Framework relocated to `~/workspace/AgentForge/` (canonical path), compiled successfully, committed to `https://github.com/JPeetz/agentforge.git` (main branch, commit aae707c). Binary verified: `AgentForge 0.2.0 (dev)` — 14MB Mach-O x86_64. The architecture spec and scaffolding were real; the path was wrong. Paper-launch retracted.

  **Additional findings from the audit:**
  1. Obsidian vault (118 files) has 94.9% isolation rate — only 4 of 118 files have any wiki-links. Vault linker agent deployed to fix.
  2. MeMex ↔ Obsidian sync is broken: duplicate files (closed-learning-loop, pipeline-log, standby-agents) have diverged. Memory sync agent deployed.
  3. Key MeMex files (decisions, seo-patterns, memory-architecture, learning-log) missing from Obsidian. Being copied.
  4. CodeFlow repo assessed as useful visualization utility (Obsidian graph view) but not automation-integrable. Recommended for vault visualization.

  **CEO directive:** CodeFlow should be used going forward for agentforge development as a visual audit tool — pointing it at the vault and Go codebase surfaces structural issues (orphaned notes, circular deps, security flags) that are invisible in flat file view. Add to CI when the framework ships publicly.
